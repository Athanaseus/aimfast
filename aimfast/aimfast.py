...
def get_box(wcs, radec, w):
    ...
    decPix = int(decPix)
    box = (
        slice(decPix - int(w / 2), decPix + int(w / 2)),
...
def residual_image_stats(
    fitsname, test_normality=None, data_range=None, threshold=None, chans=None, mask=None
):
    ...
    data = residual_data[0] if residual_data.shape[0] == 1 else residual_data[1]
    if chans:
        nchans = []
...
def normality_testing(data, test_normality="normaltest", data_range=None):
    ...
    if len(res_data) == 0:
        raise ValueError(
            f"{R}No data to compute stats.\nEither threshold too high or all data is masked.{{W}}"
        )
    if data_range:
...
def model_dynamic_range(lsmname, fitsname, beam_size=5, area_factor=2):
    ...
    try:
        sources_flux = dict(
            [(model_source, model_source.getTag("I_peak")) for model_source in model_sources]
        )
        peak_source_flux = [
            (_model_source, flux)
            for _model_source, flux in sources_flux.items()
            if flux == max(list(sources_flux.values()))
        ][0][0]
    ...
    if not peak_flux:
        ...
        sources_flux = dict([(model_source, model_source.flux.I) for model_source in model_sources])
        peak_source_flux = [
            (_model_source, flux)
            for _model_source, flux in sources_flux.items()
            if flux == max(list(sources_flux.values()))
...
def get_model(catalog, mappings=None):
    ...
    def tigger_src_nvss(src, idx):
        ...
        flux = ModelClasses.Polarization(
            float(src["S1.4"] / 1000.0), 0, 0, 0, I_err=float(src["e_S1.4"] / 1000.0)
        )
        ra, ra_err = map(
            np.deg2rad, (float(ra2deg(src["RAJ2000"])), float(src["e_RAJ2000"] / 3600.0))
        )
        dec, dec_err = map(
            np.deg2rad, (float(dec2deg(src["DEJ2000"])), float(src["e_DEJ2000"] / 3600.0))
        )
        pos = ModelClasses.Position(ra, dec, ra_err=ra_err, dec_err=dec_err)
        ...
        # Adding source peak flux (error) as extra flux attributes for sources,
        # and to avoid null values for point sources I_peak = src["Total_flux"]
        source.setAttribute("I_peak", float(src["S1.4"] / 1000.0))
        source.setAttribute("I_peak_err", float(src["e_S1.4"] / 1000.0))
        return source
    ...
    def tigger_src_sumss(src, idx):
        ...
        name = "SRC%d" % idx
        flux = ModelClasses.Polarization(
            float(src["St"] / 1000.0), 0, 0, 0, I_err=float(src["e_St1.4"] / 1000.0)
        )
        ra, ra_err = map(
            np.deg2rad, (float(ra2deg(src["RAJ2000"])), float(src["e_RAJ2000"] / 3600.0))
        )
    ...
    def tigger_src_fits(src, idx, freq0=None):
        ...
        name = "SRC%d" % idx
        flux = ModelClasses.Polarization(
            float(src["Total_flux"]), 0, 0, 0, I_err=float(src["E_Total_flux"])
        )
    ...
    def tigger_src_wsclean(src, idx):
        ...
        name = src["col1"]
        flux = ModelClasses.Polarization(float(src["col5"]), 0, 0, 0, I_err=float(0.00))
        ra, ra_err = map(np.deg2rad, (float(ra2deg(src["col3"])), float(0.00)))
        dec, dec_err = map(np.deg2rad, (float(dec2deg(src["col4"])), float(0.00)))
        pos = ModelClasses.Position(ra, dec, ra_err=ra_err, dec_err=dec_err)
        ex, ex_err = map(
            np.deg2rad,
            (
                float(src["col9"]) if type(src["col9"]) is not np.ma.core.MaskedConstant else 0.00,
                float(0.00),
            ),
        )
        ey, ey_err = map(
            ...
            (
                float(src["col10"])
                if type(src["col10"]) is not np.ma.core.MaskedConstant
                else 0.00,
                float(0.00),
            ),
        )
...
def get_detected_sources_properties(
    ...
    for i, model1_source in enumerate(model1_sources):
        ...
        if closest_only:
            if len(model2_sources) > 1:
                rdist = np.array(
                    [
                        _source_angular_dist_pos_angle(model1_source, model2_source)[0]
        ...
        if I_out_list[0] > 0.0:
            ...
            if closest_only:
                ...
                try:
                    ...
                    I_out_err = sum([1.0 / I_out_error for I_out_error in I_out_err_list])
                    I_out_var_err = np.sqrt(1.0 / I_out_err)
                    I_out /= I_out_err
                    I_out_err = I_out_var_err
                    ra2 = np.sum([src.pos.ra * src.flux.I for src in model2_sources]) / np.sum(
                        [src.flux.I for src in model2_sources]
                    )
                    dec2 = np.sum([src.pos.dec * src.flux.I for src in model2_sources]) / np.sum(
                        [src.flux.I for src in model2_sources]
                    )
                    ...
                    ra_err2 = sorted(model2_sources, key=lambda x: x.flux.I, reverse=True)[
                        0
                    ].pos.ra_err
                    dec_err2 = sorted(model2_sources, key=lambda x: x.flux.I, reverse=True)[
                        0
                    ].pos.dec_err
                    ...
                    if len(model2_sources) > 1:
                        ...
                        rdist = np.array(
                            [
                                _source_angular_dist_pos_angle(model2_source, model1_source)[0]
                                for model2_source in model2_sources
                            ]
                        )
                        model2_sources = [model2_sources[np.argmin(rdist)]]
            ...
            if delta_phase_centre_arc_sec is None or delta_phase_centre_arc_sec <= deg2arcsec(
                ...
                LOGGER.info(
                    f"Source {source1_name} will be unmatched "
    ...
    return (
        ...
        targets_scale,
        targets_position,
        targets_not_matching_a,
        targets_not_matching_b,
        sources_overlay,
    )
    ...
    for s1 in sources1:
        if s1.name not in matched_names.keys():
            props1 = [
                ...
                round(s1.flux.I_err * FLUX_UNIT_SCALER[units][0], deci) if s1.flux.I_err else None,
                unwrap(round(rad2deg(s1.pos.ra), deci)),
                f"{rad2deg(s1.pos.ra_err):.{deci}e}" if s1.pos.ra_err else None,
                round(rad2deg(s1.pos.dec), deci),
                f"{rad2deg(s1.pos.dec_err):.{deci}e}" if s1.pos.dec_err else None,
            ]
    ...
    for s2 in sources2:
        if s2.name not in matched_names.values():
            props2 = [
                ...
                round(s2.flux.I_err * FLUX_UNIT_SCALER[units][0], deci) if s2.flux.I_err else None,
                unwrap(round(rad2deg(s2.pos.ra), deci)),
                f"{rad2deg(s2.pos.ra_err):.{deci}e}" if s2.pos.ra_err else None,
                round(rad2deg(s2.pos.dec), deci),
                f"{rad2deg(s2.pos.dec_err):.{deci}e}" if s2.pos.dec_err else None,
            ]
...
def plot_photometry(
    ...
    for model1, model2 in models.items():
        _models.append(
            [
                dict(label="{}-model_a_{}".format(label, i), path=model1),
...
def plot_residuals_noise(res_noise_images, skymodel=None, label=None, area_factor=2.0, points=100):
    ...
    for res1, res2 in res_noise_images.items():
        _residual_images.append(
            [
                dict(label="{}-res_a_{}".format(label, i), path=res1),
    ...
    for pair, model_pair in enumerate(all_models):
        ...
        if len(flux_in_data) > 1:
            ...
            if plot_type == "inout":
                ...
                yerr1 = yerr
                axis_labels = [
                    f"{model_1_name} S1 ({FLUX_UNIT_SCALER[units][1]})"
                    if not xlabels
                    else xlabels[pair],
                ...
                yerr1 = yerr
                axis_labels = [
                    "log S1" if not xlabels else xlabels[pair],
            ...
            for xval, yval, xerr, yerr in zip(
                x1,
                y1,
            ...
            dtab = DataTable(
                source=source, columns=columns, width=500, max_width=550, height=100, max_height=150
            )
            ...
            stats1 = {
                "Source": [s[0] for s in no_match1],
                "Flux [%s]" % _fu: [s[1] for s in no_match1],
                "Flux_err [%s]" % _fu: [s[2] for s in no_match1],
                "RA": [deg2ra(s[3], deci) for s in no_match1],
                "RA_err ['']": [round(deg2arcsec(s[4] if s[4] else 0), deci) for s in no_match1],
                "DEC": [deg2dec(s[5], deci) for s in no_match1],
                "DEC_err ['']": [round(deg2arcsec(s[6] if s[6] else 0), deci) for s in no_match1],
            }
            ...
            # Append all plots
            flux_plot_list.append(
                column(row(plot_flux, column(stats_table, stats_table1, stats_table2)))
            )
        else:
    ...
    for model_pair in all_models:
        ...
        if len(flux_in_data) > 1:
            ...
            plot_position = figure(
                ...
                x_axis_label="RA offset ({:s})".format(POSITION_UNIT_SCALER["arcsec"][1]),
                y_axis_label="DEC offset ({:s})".format(POSITION_UNIT_SCALER["arcsec"][1]),
                tools=TOOLS,
            )
            ...
            plot_overlay = figure(
                title="Catalogs Overlay",
                x_axis_label="RA ({:s})".format(POSITION_UNIT_SCALER["deg"][1]),
                y_axis_label="DEC ({:s})".format(POSITION_UNIT_SCALER["deg"][1]),
                match_aspect=True,
            ...
            error1_plot = plot_position.multi_line(
                err_xs1, err_ys1, legend_label="Errors", color="red"
            )
            ...
            columns = [TableColumn(field=x, title=x.capitalize()) for x in cols]
            dtab = DataTable(
                source=source, columns=columns, width=450, max_width=500, height=100, max_height=150
            )
            table_title = Div(text="Cross Matching Statistics")
            table_title.align = "center"
            stats_table = column([table_title, dtab])
            ...
            hover.tooltips = OrderedDict(
                [
                    ("source", "(@label)"),
                    ("(S1,S2) [mJy]", "(@flux_s1, @flux_s2)"),
                    ("(RA,DEC)", "(@ra_dec)"),
                    ("(RA_err,DEC_err)", "(@ra_err, @dec_err)"),
                    ("(RA_offset,DEC_offset)", "(@ra_offset, @dec_offset)"),
                    ("Distance off-axis", "@phase_centre_dist"),
                ]
            )
            plot_overlay.add_tools(
                HoverTool(
                    renderers=[plot_overlay_1],
                    tooltips=OrderedDict(
                        [
                            ("source1", "@label1"),
                            ("Flux (mJy)", "@flux1"),
                            ("(RA,DEC)", "(@str_ra1, @str_dec1)"),
                            ("(RA_err,DEC_err)", "(@ra_err1, @dec_err1)"),
                        ]
                    ),
                )
            )
            plot_overlay.add_tools(
                HoverTool(
                    renderers=[plot_overlay_2],
                    tooltips=OrderedDict(
                        [
                            ("source2", "@label2"),
                            ("Flux (mJy)", "@flux2"),
                            ("(RA,DEC)", "(@str_ra2, @str_dec2)"),
                            ("(RA_err,DEC_err)", "(@ra_err2, @dec_err2)"),
                        ]
                    ),
                )
            )
...
def _random_residual_results(res_noise_images, data_points=None, fov_factor=None, area_factor=None):
    ...
    for images in res_noise_images:
        ...
        nchan1 = res_data1.shape[1] if res_data1.shape[0] == 1 else res_data1.shape[0]
        nchan2 = res_data2.shape[1] if res_data2.shape[0] == 1 else res_data2.shape[0]
...
def _source_residual_results(res_noise_images, skymodel, area_factor=None):
    ...
    for images in res_noise_images:
        ...
        for model_source in model_sources:
            ...
            phase_centre_dist = phase_dist_arcsec / 3600.0
            # Get width of box around source
            width = int(deg2arcsec(beam_deg[0]) * area_factor)
            # Get a image slice around source
            imslice = get_box(fits_info["wcs"], (RA, DEC), width)
...
def plot_aimfast_stats(fidelity_results_file, units="micro", prefix=""):
    ...
    variance_plotter.line(
        im_keys, np.array(stddev_values) * multiplier, legend_label="std", color="blue"
    )
    variance_plotter.line(
        im_keys, np.array(mad_values) * multiplier, legend_label="mad", color="red"
    )
    variance_plotter.line(
        im_keys, np.array(max_values) * multiplier, legend_label="max", color="green"
    )
    variance_plotter.title.align = "center"

    # Moment 3 & 4 plots
    mom34_plotter = figure(
        x_range=im_keys,
        x_axis_label="Image",
        y_axis_label="Value",
        width=width,
    ...
    output_file(outfile)
    save(column(row(variance_plotter, mom34_plotter), row(norm_plotter, dr_plotter)), title=outfile)


def plot_subimage_stats(
    fitsnames,
    centre_coords,
    sizes,
    htmlprefix="default",
    ...
    for im in range(len(centre_coords)):
        ...
        for n, fitsname in enumerate(fitsnames):
            ...
            sub_stats = {
                "RMS": round(subimg_stats["RMS"] * FLUX_UNIT_SCALER[units][0], DECIMALS),
                "STDDev": round(subimg_stats["STDDev"] * FLUX_UNIT_SCALER[units][0], DECIMALS),
                "MAD": round(subimg_stats["MAD"] * FLUX_UNIT_SCALER[units][0], DECIMALS),
                "MIN": round(subimg_stats["MIN"] * FLUX_UNIT_SCALER[units][0], DECIMALS),
                "MAX": round(subimg_stats["MAX"] * FLUX_UNIT_SCALER[units][0], DECIMALS),
                "SUM_NEG": round(subimg_stats["SUM_NEG"] * FLUX_UNIT_SCALER[units][0], DECIMALS),
            ...
            columns = [TableColumn(field=x, title=x.capitalize()) for x in cols]
            dtab = DataTable(
                source=source, columns=columns, width=250, max_width=350, height=200, max_height=250
            )
            ...
            if len(im_subplot_list) > 0:
                ...
                subplot = figure(
                    ...
                    tooltips=[
                        ("(x, y)", "($x, $y)"),
                        (f"value ({FLUX_UNIT_SCALER[units][1]})", "@image"),
                    ],
                )
            else:
            ...
            if svg:
                ...
                try:
                    ...
                    shw = plt.imshow(
                        subimage * FLUX_UNIT_SCALER[units][0],
                    ...
                    plt.xlabel(
                        "Right Ascension (hours)", fontsize=float(x_label_size.split("pt")[0])
                    )
                    plt.ylabel("Declination (deg)", fontsize=float(y_label_size.split("pt")[0]))
                    bar.set_label(
                        f"Flux density ({FLUX_UNIT_SCALER[units][1]})",
            ...
            subplot.image(
                image=[subimage * FLUX_UNIT_SCALER[units][0]],
                x=rx_0,
                y=ry_0,
                dw=size,
                dh=size,
            ...
            color_bar_plot = figure(
                title=f"Flux Density ({FLUX_UNIT_SCALER[units][1]})",
...
def get_source_properties_from_catalog(catalog_file):
    ...
    for source in sources:
        ...
        try:
            ...
            shapes = ["ex=0", "ey=0", "pa=0", "ex_err=0", "ey_err=0", "pa_err=0"]
        for shape in shapes:
            prop = shape.split("=")[0]
...
def get_argparser():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Examine radio image fidelity and source recovery by obtaining: \n"
    ...
    argument("--residual-image", dest="residual", help="Name of the residual image fits file")
    argument("--mask-image", dest="mask", help="Name of the mask image fits file")
    argument(
        "-fdr",
        "--fidelity-results",
        dest="json",
    ...
    argument(
        "-reg", "--input-regions", dest="reg", help="Region file with regions to generate stats)"
    )
    # Source finding
    argument(
        "-c",
    ...
    argument(
        "--online-catalog-name", dest="catalog_name", help="Prefix of output catalog file name"
    )
    argument(
        "-oc",
        "--online-catalog",
    ...
    argument(
        ...
        "--width",
        dest="width",
        help="Field of view width to querry online catalog in degrees.e.g. -w 3.0d",
    )
    ...
    argument(
        "-dr", "--data-range", dest="data_range", help="Data range to perform normality testing"
    )
    argument(
        "-thresh",
        "--threshold",
    ...
    # Formatting
    argument(
        "-dp", "--data-points", dest="points", help="Data points to sample the residual/noise image"
    )
    argument(
        "-fp",
        "--flux-plot",
    ...
    argument("-x", "--x-col-data", dest="x_col", help="Catalog column name to plot on the x-axis")
    argument("-y", "--y-col-data", dest="y_col", help="Catalog column name to plot on the y-axis")
    argument(
        "-x-err",
        "--x-col-err-data",
        dest="x_col_err",
    ...
    argument("-x-label", "--x-label", dest="x_label", help="x-axis labels for the plot")
    argument("-y-label", "--y-label", dest="y_label", help="y-axis labels for the plots")
    argument("-title", "--plot-title", dest="title", help="Title label for the basic catalog plot")
    # Plot labelling for the flux comparison plotting
    argument(
        "-fx", "--flux-xlabels", dest="fxlabels", nargs="+", help="x-axis labels for the Flux plots"
    )
    argument(
        "-fy", "--flux-ylabels", dest="fylabels", nargs="+", help="y-axis labels for the Flux plots"
    )
    argument(
        "-ftitle",
        "--flux-plot-title",
    ...
    # Outputs
    argument("--html-prefix", dest="htmlprefix", help="Prefix of output html files. Default: None.")
    argument("--outfile", help="Name of output file name. Default: fidelity_results.json")
    argument(
        "-svg",
        "--save-svg",
        dest="svg",
...
def main():
    ...
    if args.subcommand:
        ...
        not args.residual
        and not args.restored
        and not args.model
        and not args.models
        and not args.noise
        and not args.images
        and not args.subimage_noise
        and not args.online
        and not args.json
    ):
    ...
    if args.model and not args.noise and args.residual:
        ...
        if args.psf:
            ...
            psf_size = 6
            LOGGER.warning(
                f"{R}Please provide psf fits file or psf size.\n"
    ...
    if args.models:
        ...
        if len(models) < 1:
            ...
            for i, comp_mod in enumerate(models):
                ...
                models_list.append(
                    [
                        dict(label="{}-model_a_{}".format(args.label, i), path=model1),
                        dict(label="{}-model_b_{}".format(args.label, i), path=model2),
                    ],
                )
        ...
        if len(residuals) < 1:
            ...
            for i, comp_res in enumerate(residuals):
                ...
                residuals_list.append(
                    [
                        dict(label="{}-res_a_{}".format(args.label, i), path=res1),
                        dict(label="{}-res_b_{}".format(args.label, i), path=res2),
                    ],
                )
            if args.model:
        ...
        for i, comp_ims in enumerate(images):
            ...
            images_list.append(
                [
                    ...
                    dict(label="{}-model_b_{}".format(args.label, i), path=out2),
                ]
            )
        output_dict = compare_models(
            images_list,
    ...
    if args.online:
        ...
        table = get_online_catalog(
            catalog=online_catalog.upper(),
        ...
        if table:
            for i, ims in enumerate(models):
                ...
                images_list.append(
                    [
                        dict(label="{}-model_a_{}".format(args.label, i), path=image1),
                        dict(label="{}-model_b_{}".format(args.label, i), path=catalog_name),
                    ]
                )
    ...
    if args.subimage_noise:
        ...
        if args.centre_pix_size:
            ...
            output_dict = plot_subimage_stats(
                ...
                htmlprefix=(args.htmlprefix if args.htmlprefix else "default"),
            )
        else:
            LOGGER.error(f"{R}Provide Centre coordinates in pixels and size of subimage(s).{W}")
